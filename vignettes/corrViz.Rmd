...
The `corrViz` package offers several visualization methods to help you explore and communicate correlations effectively, with an emphasis on interactivity. At it's core, `corrViz` uses the in-built `R` function `cor()` to compute the correlations. As a result any of the correlation methods available in the `cor` function can be used in the `corrViz` visualisations via the `method` argument. The only required inputs for all `corrViz` functions are the a dataset and the correlation method. However, each visualization has multiple customization options, which are outlined in this document. The `plotly` package^[ C. Sievert. Interactive Web-Based Data Visualization with R, plotly, and shiny. Chapman and Hall/CRC Florida, 2020.] is used to create interactive graphics.
...
```
<center>
<caption><span id="fig2:fig2">Figure 2: </span>Interactive heatmap displaying correlations from
the lower section of the heatmap.</caption>
</center>
...
In a solar system correlation plot, the _dependent_ (sun) variable of interest is positioned at the center, represented as the sun. The _explanatory_ (planet) variables are depicted as planets orbiting around the sun, with their distance from the sun corresponding to the absolute value of their correlation with the dependent variable. Therefore, the greater the distance of a planet from the sun, the weaker the correlation between the explanatory variable and the dependent variable. Figure 5 shows a static solar system correlation plot with `mpg` selected as the dependent variable via the `sun` argument. Planet variables are coloured either red (positive correlation) or blue (negative correlation).

In Figure 5, the variable `mpg` is selected as the sun and is placed in the centre. 
...
```
<center>
<caption><span id="fig5:fig5">Figure 5: </span>Solar system correlation plot with the variable `mpg` as the sun.</caption>
</center>

# Solar plot animation

In addition to the static solar system correlation plot, we provide an animated version which displays the planet variables orbiting the sun. The additional arguments are:
...
```
<center>
<caption><span id="fig6:fig6">Figure 6: </span>Animated solar system correlation plot with the variable `mpg` as the sun.</caption>
</center>
...
Figure 7 shows an interactive barplot displaying correlation values. The red bars extending right represent the positive correlations whereas the blue bars extending left represent the negative correlations. 
...
```
<center>
<caption><span id="fig7:fig7">Figure 7: </span>Bar plot displaying correlation values.</caption>
</center>
...
Figure 8 is similar to the heatmap shown in Figures 1 and 2, except this time each variable pair is represented as a circle, with the size corresponding the the magnitude of the correlation. Hovering over a bubble will display the variable pair names and their associated correlation. The additional arguments for this function are:
...
```
<center>
<caption><span id="fig8:fig8">Figure 8: </span>Bubble plot displaying correlation values.</caption>
</center>

As with the heatmap, we can also display a portion of the plot using the `display` argument, as shown in Figure 9.
...
```
<center>
<caption><span id="fig9:fig9">Figure 9: </span>Bubble plot displaying correlation values from the upper portion of the correlation matrix.</caption>
</center>
...
Figure 10 shows an interactive scatterplot for the first 5 variables found in the `mtcars` dataset. In this case, the scatterplot points are coloured by the variable `cyl`. 
...
```
<center>
<caption><span id="fig11:fig11">Figure 11: </span>Sankey plot displaying correlation values with a magnitude above 0.6.</caption>
</center>


Setting `colour = TRUE` colours the links based on their correlation. In figure 12, the `threshold` argument is set to equal 0.8.
...
```
<center>
<caption><span id="fig12:fig12">Figure 12: </span>Coloured Sankey plot displaying correlation values with a magnitude above 0.8.</caption>
</center>

# Chord plot

Figure 13 shows a chord plot displaying correlations. This type of plot can quickly become overloaded with information, so it is recommended to only use this plot when thresholding correlation values. In Figure 13, `threshold = 0.8`, which filters any correlations from the visualization an absolute value lower than selected value. The width of the chords corresponds to the magnitude of the correlation with their colours indicating positive (red) or negative correlations (blue).
The additional arguments are:
...
```
<center>
<caption><span id="fig13:fig13">Figure 13: </span>Chord plot displaying correlation values with a magnitude above 0.8.</caption>
</center>

Figure 14, displays the same information as Figure 13, but uses a circular layout, via the `circle` argument.
...
```
<center>
<caption><span id="fig14:fig14">Figure 14: </span>Circular chord plot displaying correlation values with a magnitude above 0.8.</caption>
</center>

# Circular correlation plot

Figure 15 shows a circular correlation plot. This is similar to Figure 14 but places the variable names on the outside of a neater circle. As with Figures 13 and 14, the width of the chords in Figure 15 corresponds to the magnitude of the correlation with their colours indicating positive (red) or negative correlations (blue). The additional arguments are:
...
```
<center>
<caption><span id="fig15:fig15">Figure 15: </span>Circular correlation plot displaying correlation values.</caption>
</center>

In Figure 16, we filter the visualization to display correlations with an absolute value greater than 0.8. In this case the axis ticks are removed.
...
```
<center>
<caption><span id="fig16:fig16">Figure 16: </span>Circular correlation plot displaying correlation values with a magnitude above 0.8.</caption>
</center>
