...
In addition to interactive plots, `corrViz` also provides numerous static plots. In Figures 12 to 15 we show a selection of options to display correlations on a grid. The plot types can be any of _square_, _circle_, _text_, or _pie_, which can be set via the `type` argument. If the `type` argument is set to either `square` or `circle`, then the shapes are sized, with the size representing the absolute value of the correlation. 
The additional arguments are: 
...
In Figure 12, we set `type = 'square'`

```{r, square, fig.align='center',out.width='100%', out.height='100%'}
corrGrid(data = mtcars[1:7],
         type = 'square')

```
<center>
<caption><span id="fig12:fig12">Figure 12: </span>Static heatmap correlation plot.</caption>
</center>

In Figure 13, we set `type = 'circle'`.
...
```
<center>
<caption><span id="fig13:fig13">Figure 13: </span>Static circle correlation plot.</caption>
</center>


In Figure 14, we set `type = 'text'`. The text 
...
```
<center>
<caption><span id="fig14:fig14">Figure 14: </span>Static text correlation plot.</caption>
</center>

# Correlation Pie Plot
In Figure 15, we set `type = 'pie'`. This plot shows pie charts where the filled in percentage represents the correlation value.

```{r, pie, fig.align='center', out.width='100%', out.height='100%'}
corrGrid(data = mtcars[1:7],
         type = 'pie')

```
<center>
<caption><span id="fig15:fig15">Figure 15: </span>Static pie-chart correlation plot.</caption>
</center>



# Static Solar Plot
Figure 16 shows a static version of the solar system correlation plot with `mpg` selected as the dependent variable via the `sun` argument. Planet variables are coloured either red (positive correlation) or blue (negative correlation).
...
```
<center>
<caption><span id="fig16:fig16">Figure 16: </span>Solar system correlation plot with the variable `mpg` as the sun.</caption>
</center>
...
```
<center>
<caption><span id="fig17:fig17">Figure 17: </span>Chord plot displaying correlation values with a magnitude above 0.8.</caption>
</center>

Figure 18, displays the same information as Figure 17, but uses a circular layout, via the `circle` argument.
...
```
<center>
<caption><span id="fig18:fig18">Figure 18: </span>Circular chord plot displaying correlation values with a magnitude above 0.8.</caption>
</center>

# Circular correlation plot

Figure 19 shows a circular correlation plot. This is similar to Figure 18 but places the variable names on the outside of a neater circle. As with Figures 17 and 18, the width of the chords in Figure 15 corresponds to the magnitude of the correlation with their colours indicating positive (red) or negative correlations (blue). The additional arguments are:
...
```
<center>
<caption><span id="fig19:fig19">Figure 19: </span>Circular correlation plot displaying correlation values.</caption>
</center>

In Figure 20, we filter the visualization to display correlations with an absolute value greater than 0.8. In this case the axis ticks are removed.
...
```
<center>
<caption><span id="fig20:fig20">Figure 20: </span>Circular correlation plot displaying correlation values with a magnitude above 0.8.</caption>
</center>
