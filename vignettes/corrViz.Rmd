...
Figure 10 shows a chord plot displaying correlations. This type of plot can quickly become overloaded with information, so it is recommended to only use this plot when thresholding correlation values. In Figure 10, `threshold = 0.8`, which filters any correlations from the visualization an absolute value lower than selected value. The width of the chords corresponds to the magnitude of the correlation with their colours indicating positive (red) or negative correlations (blue).
...
```
<center>
<caption><span id="fig10:fig10">Figure 10: </span>Chord plot displaying correlation values with a magnitude above 0.8.</caption>
</center>

Figure 11, displays the same information as Figure 10, but uses a circular layout, via the `circle` argument.
...
```
<center>
<caption><span id="fig11:fig11">Figure 11: </span>Circular chord plot displaying correlation values with a magnitude above 0.8.</caption>
</center>

# Circular correlation plot

Figure 12 shows a circular correlation plot. This is similar to Figure 11 but places the variable names on the outside of a neater circle. as with Figures 10 and 11, the width of the chords in Figure 12 corresponds to the magnitude of the correlation with their colours indicating positive (red) or negative correlations (blue). The additional arguments are:
...
```
<center>
<caption><span id="fig12:fig12">Figure 12: </span>Circular correlation plot displaying correlation values.</caption>
</center>

In Figure 13, we filter the visualization to display correlations with an absolute value greater than 0.8. In this case the axis ticks are removed.
...
Figure 14 shows an interactive scatterplot for the first 5 variables found in the `mtcars` dataset. In this case, the scatterplot points are coloured by the variable `cyl`. 
...
```
<center>
<caption><span id="fig14:fig14">Figure 14: </span>Correlation pairs plot for the first five variables from the `mtcars` dataset.</caption>
</center>
